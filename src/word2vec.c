...
int multitask = 0, task_specific = 0, least_sq = 0;
...
static void usage(int a_ret) {
  ...
  printf("\tTrain multi-task embeddings (1-st line of the training file should contain declaration)\n");
...
void ReadVocab(const char train_file[]) {
  long long a, i = 0;
  char c;
  char word[MAX_STRING];
  ...
  while (1) {
    ...
    if (feof(fin)) break;
    a = AddWordToVocab(word);
    if (fscanf(fin, "%lld%c", &vocab[a].cn, &c) != 2) {
...
void *TrainModelThread(void *opts) {
  long long a, b, d, cw, word, last_word, sentence_length = 0, sentence_position = 0;
  long long word_count = 0, last_word_count = 0, sen[MAX_SENTENCE_LENGTH + 1];
  long long l1, l2, c, target, label, local_iter = iter;
  unsigned long long next_random = ((pthread_opts_t *) opts)->m_id;
  real f, g;
  clock_t now;
  real *neu1 = (real *)calloc(layer1_size, sizeof(real));
  real *neu1e = (real *)calloc(layer1_size, sizeof(real));
  FILE *fi = fopen(((pthread_opts_t *) opts)->m_train_fname, "rb");
...
void train_model(const char train_file[], const char output_file[]) {
  long a, b, c, d;
  ...
  FILE *fo = output_file[0]? fopen(output_file, "wb"): stdout;

  if (read_vocab_file[0] != 0)
    ReadVocab(train_file);
  else
  ...
  pthread_opts_t opts;
  opts.m_train_fname = (const char *) train_file;
  for (a = 0; a < num_threads; ++a) {
    opts.m_id = a;
    pthread_create(&pt[a], NULL, TrainModelThread, (void *) &opts);
  }
  for (a = 0; a < num_threads; a++)
