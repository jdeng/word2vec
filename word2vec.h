...
// MIT License
//
// Based on Google Word2Vec and gensim from Radim Rehurek (see
// http://radimrehurek.com/2013/09/deep-learning-with-word2vec-and-gensim/)
// See `main.cc` for building instructions
//
// P.S. compiler vector extension, AVX intrinsics (even with FMA)
// doesn't seem to help compared to compilation with -Ofast
// -march=native on a haswell laptop
...
template <class String = std::string>
  ...
  :layer1_size_(size), window_(window), sample_(sample), min_count_(min_count), negative_(negative)
	...
	printf("collecting [%s] %lu sentences, %zu distinct %ss, %d %ss\n", type, count, vocab.size(), type,
